...
·注解：在编码过程中，为了更好的理解程序，我们通常会加上注释代码，这样可以方便程序员理解。注解就是注释的一种，但是与一般的注释不同的是，注解需要按一定规则编写（见自定义注解），并且注解可以被程序解析，并根据解析结果进行相应处理。如果没有用来读取注解的方法和工作，那么注解也就不会比注释更有用处了。使用注解的过程中，很重要的一部分就是创建于使用注解处理器。Java SE5扩展了反射机制的API，以帮助程序员快速的构造自定义注解处理器。
·元数据：指用来描述数据的数据。
...
·抽象的最佳实践：
  通过接口增加一层抽象虽然具备更好的可扩展性，但是过渡设计会导致编码思维不连续性，效率低，不易读，修改成本高等问题。一般的的业务代码可以参考如下原则： 
  -- 代码分层，上层依赖下层（依赖结构更清晰）
  -- 低层越稳定越好（低层稳定，意味着依赖稳定）
  -- 尽量单向依赖，不能保证单向依赖才依赖抽象（避免开发低效，代码结构凌乱）。例如同层模块之间依赖复杂，可借助中介者模式简化依赖。
·文档：
